<?php
	...
	function usuarioExiste($usuario)
	{
		...
		$stmt = $mysqli->prepare("SELECT id_inicio FROM inicio WHERE n_Guia = ? LIMIT 1");
		$stmt->bind_param("s", $n_Guia);
		$stmt->execute();
		$stmt->store_result();
		$num = $stmt->num_rows;
		$stmt->close();
	...
	function hashPassword($n_Guia)
	{
		$hash = password_hash($n_Guia, PASSWORD_DEFAULT);
		return $hash;
	}
	...
	function registraUsuario($n_Guia, $chofer, $camion, $fecha, $viaje){
		
		global $mysqli;
		
		$stmt = $mysqli->prepare("INSERT INTO inicio (n_Guia, chofer, camion, fecha, viaje) VALUES (?, ?, ?, ?, ?)");
		$stmt->bind_param('ssssisi', $n_Guia, $chofer, $camion, $fecha, $viaje);
	...
	function validaIdToken($id_inicio, $fecha){
		global $mysqli;
		
		$stmt = $mysqli->prepare("SELECT camion FROM inicio WHERE id_inicio = ? AND fecha = ? LIMIT 1");
		$stmt->bind_param("is", $id_inicio, $fecha);
		$stmt->execute();
		$stmt->store_result();
		$rows = $stmt->num_rows;
	...
	function activarUsuario($id)
	{
		...
		$stmt = $mysqli->prepare("UPDATE inicio SET fecha=now() WHERE id_inicio = ?");
	...
	*/
	function isActivo($camion)
	{
		global $mysqli;
		
		$stmt = $mysqli->prepare("SELECT fecha FROM inicio WHERE camion = ? || viaje = ? LIMIT 1");
		$stmt->bind_param('ss', $camion, $camion);
		$stmt->execute();
		$stmt->bind_result($activacion);
		$stmt->fetch();
